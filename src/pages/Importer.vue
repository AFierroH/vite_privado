    ...
    <section v-if="parsedTables.length" class="mb-6 p-4 border rounded">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <select v-model="selectedDestTable" class="w-full p-2 border rounded text-black bg-white" @change="onDestChange">
        ...
        <div>
          ...
          <select v-model="selectedSourceTable" class="w-full p-2 border rounded text-black bg-white" @change="onSourceChange">
      ...
      <div v-if="selectedDestTable && selectedSourceTable" class="mt-4">
        ...
        <div class="mt-2 space-y-2">
          <div v-for="destField in destSchema[selectedDestTable]" :key="destField" class="flex gap-2 items-center">
            ...
            <select v-model="mapping[destField]" class="flex-1 p-2 border rounded text-black bg-white">
            ...
            <div v-if="mapping[destField] === '__static'">
              <input v-model="staticValues[destField]" placeholder="valor estático" class="p-1 border rounded text-black bg-white" />
            </div>
          </div>
        </div>
...
async function upload() {
  ...
  const res = await fetch(`${API_URL}/import/upload-sql`, {
  method: 'POST',
  body: form,
});
  ...
  if (data.uploadId) {
    ...
    const parsed = await (await fetch(`${API_URL}/import/parsed/${data.uploadId}`)).json();
    parsedTables.value = parsed;
    const schema = await (await fetch(`${API_URL}/import/dest-schema`)).json();
